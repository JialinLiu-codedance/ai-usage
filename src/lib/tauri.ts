  ...
  return repositories.flatMap((repository, index) => {
    ...
    const commits: GitUsageCommit[] = [
      {
        ...
        parent_count: 1,
        patch_id: `patch-${index}-a`,
        duplicate_group_id: `${repository.path}::patch-${index}-a`,
        duplicate_group_size: index === 0 ? 2 : 1,
        is_group_representative: true,
        commit_role: index === 0 ? "pr_merge" : "original",
    ...
    if (index < 2) {
      commits.push({
        ...
        repository_path: repository.path,
        parent_count: 1,
        patch_id: index === 0 ? `patch-${index}-a` : `patch-${index}-b`,
        duplicate_group_id: index === 0 ? `${repository.path}::patch-${index}-a` : `${repository.path}::patch-${index}-b`,
        duplicate_group_size: index === 0 ? 2 : 1,
        is_group_representative: index !== 0,
        commit_role: index === 0 ? "original" : "original",
