...
pub fn anthropic_to_openai(body: Value) -> Result<Value, LocalProxyTransformError> {
    let mut result = json!({});
...
) -> Result<Vec<Value>, LocalProxyTransformError> {
...
pub fn openai_to_anthropic(body: Value) -> Result<Value, LocalProxyTransformError> {
    let choices = body
        .get("choices")
